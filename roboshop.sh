...
do
        ...
        # Use string comparison and quote the variable. If instance is not 'frontend' use private IP.
        if [ "$instance" != "frontend" ]; then
                IP=$(aws ec2 describe-instances --instance-ids "$INSTANCE_ID" --query 'Reservations[0].Instances[0].PrivateIpAddress' --output text)
                # Replace spaces in instance name with hyphens for a valid DNS record name
                SANITIZED_NAME="${instance// /-}"
                RECORD_NAME="$SANITIZED_NAME.$Domain_Name"
        else
